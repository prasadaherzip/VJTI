...
typedef struct {
    ...
    Chunk chunks[64];   // Fixed size array of chunks metadata
} Page;
...
int cleanup() {
    // According to spec, do not munmap pages here
    return 0; 
}
...
static void split_chunk(Page *p, size_t idx, size_t size) {
    ...
    if (c->size > size) {
        ...
        for (size_t i = p->chunk_count; i > idx + 1; i--) {
...
char* alloc(int size) {
    if (size <= 0 || (size % ALIGNMENT) != 0 || size > PAGE_SIZE) return NULL;
    size = align256(size);
    ...
    void* new_page = mmap(NULL, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
    if (new_page == MAP_FAILED) return NULL;

    // Initialize page data
    Page* new_p = &pages[page_count++];
    new_p->page_addr = new_page;
    new_p->chunk_count = 1;
...
int dealloc(char* ptr) {
    ...
    for (size_t i = 0; i < page_count; i++) {
        ...
        if (ptr >= (char*)p->page_addr && ptr < (char*)p->page_addr + PAGE_SIZE) {
            size_t offset = ptr - (char*)p->page_addr;
            for (size_t j = 0; j < p->chunk_count; j++) {
